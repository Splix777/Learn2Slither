...
class PygameGUI(GUI):
    def __init__(self, config: Config, environment: Environment):
        ...
        self.screen = pygame.display.set_mode(self.base_resolution, pygame.RESIZABLE)
    ...
    def run(self) -> None:
        ...
        while self.running:
            ...
            if isinstance(self.screens["game"], GameScreen):
                self.screens["game"].update_map_state(self.environment.game_state)
    ...
    def render_fps(self, screen: pygame.Surface) -> None:
        ...
        fps_text: pygame.Surface = font.render(f"FPS: {int(fps)}", True, (255, 255, 255))
        screen.blit(fps_text, (self.current_resolution[0] - fps_text.get_width() - 10, 10))
